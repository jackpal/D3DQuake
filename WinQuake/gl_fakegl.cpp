...
public:
	...
	void End(){
		...
		}
		{
			...
			HRESULT hr = m_pD3DDev->SetVertexShader(m_vertexTypeDesc);
...
private:
	...
	HRESULT InitD3DX()
	{
		...
		if(windowed){
			// Backbuffer format has to match what the current desktop is displaying
...
#define LOAD_OURSELVES
	...
	void glTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width,
		...
		if ( pMipMap ) {
			...
			else if ( level >= pMipMap->GetLevelCount() ) {
				return;
			}
...
// #define USE_IMAGE_SURFACE
	...
	void glTexSubImage2D_Imp (IDirect3DTexture8* pMipMap, GLint level,
		...
		IDirect3DSurface8* pMipMapLevel;
		hr = pMipMap->GetSurfaceLevel(level, &pMipMapLevel);
		if ( FAILED(hr) ) {
			InterpretError(hr);
		}
		else {
			...
			hr = pMipMapLevel->GetDesc(&desc);

			static IDirect3DSurface8* pSurface;
			if ( pSurface ) {
				D3DSURFACE_DESC desc2;

				hr = pSurface->GetDesc(&desc2);
				if(desc2.Format != desc.Format || desc2.Width < width || desc2.Height < height){
					RELEASENULL(pSurface);
					pSurface = NULL;
				}
			}
			if(!pSurface) {
				hr = m_pD3DDev->CreateImageSurface(width, height, desc.Format,&pSurface);
			}
			else {
				hr = S_OK;
			}
			if(FAILED(hr)){
				...
				if ( FAILED(hr) ) {
					...
					hr = m_pD3DDev->CopyRects(pSurface, &sourceRect, 1, pMipMapLevel, &destPt);
					if ( FAILED(hr) ) {
						InterpretError(hr);
					}
				}
...
private:
	...
	void SetGLRenderState(){
		...
		if ( m_bViewPortDirty ) {
			...
			viewData.Y = m_windowHeight - (m_glViewPortY + m_glViewPortHeight);
